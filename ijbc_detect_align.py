...
def crop_align_images(items):
    ...
    for item in items:
        ...
        out_lm = os.path.join(args.out_dir, 'mtcnn', '%d_%s' % (subject_id, base_f)).replace(ext, '.txt')
...
proto_dir = os.path.join(args.dir, 'protocols')
gallery1_items = read_csv(os.path.join(proto_dir, 'ijbc_1N_gallery_G1.csv'))
gallery2_items = read_csv(os.path.join(proto_dir, 'ijbc_1N_gallery_G2.csv'))
probe_items = read_csv(os.path.join(proto_dir, 'ijbc_1N_probe_mixed.csv'))
